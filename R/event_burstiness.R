...
#'
#' @description
#' event_burstiness() estimates the burstiness coefficient for each of the
#' unique codes in a time series.
#'
#' @export
#'
#' @details
#' event_burstiness() returns a named vector with an estimated burstiness
#' coefficient for each unique code in the data sequence, including blank or
#' missing codes. It is assumed that the data come from a time-series and that
#' the data were recorded at a constant sampling frequency (or can be treated
#' as having been recorded at a constant sampling frequency).
#'
#' Standard errors for the resulting burstiness coefficients can be
#' obtained via boot::tsboot(ts, statistic = event_burstiness, R = 100, sim =
